...
public class XListView extends ListView implements OnScrollListener{
	...
	private boolean mIsFooterReady = false;  //到底了
		...
		if (!mEnablePullLoad) {
			...
			mFooterView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					startLoadMore();   //当点击"加载更多"文字时，加载数据
				}
			});
		}
	}
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		default:
			mLastY = -1; // reset
			if (getFirstVisiblePosition() == 0) {  //下拉刷新
				// invoke refresh
				...
				resetHeaderHeight();
			} else if (getLastVisiblePosition() == mTotalItemCount - 1) {  //上滑刷新
				// invoke load more.
				if (mEnablePullLoad
